...
contract Domains is ERC721URIStorage{
  // Magic given to us by OpenZeppelin to help us keep track of tokenIds. We're going to be using _tokenIds to keep track of the NFTs unique identifier.
  ...
  function price(string calldata name) public pure returns (uint) {//this calculates the price of the domain name by the length of the name // 1 $MATIC = 10 ** 18
    uint length = StringUtils.strlen(name);
    require(length > 0);


    if(length == 3) return 5 * 10 ** 17; // 0.5 $MATIC
    else if(length == 4) return 3 * 10 ** 17; // 0.3 $MATIC
    else return 1 * 10 ** 17; // 0.1 $MATIC
  ...
  function register(string calldata name) public payable{
    ...
    //In solidity, you also can’t combine strings directly. Instead, you have to use the encodePacked function to turn a bunch of strings into bytes and then combines them!
...
//Let’s go ahead and solidify our domain on the blockchain forever with an NFT 🤘
  ...
  //Mint it!
// we want to take our newly registered domain and create an NFT out of it.